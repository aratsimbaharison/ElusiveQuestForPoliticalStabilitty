...
# here is the clean data on WGI by country and region:
...
# the first step in the data cleaning is to drop the columns with too much NAs: HDI 3085 out of 3471 obs, GINI 2432 out of 3471 obs, povertyHeadCount 2404 out of 3471 obs
WGIdevIneqPovDF <- select(WGIdevIneqPovDF, country, iso2c, date, stability, stabilityDummy, stabilityCategory, regulatoryQuality, region, subregion, GDPannualGrowthRate, GNIperCapita)

# the second step is to imput GNI per capita using knn (It is reasonable to consider that GNI per capita does not dramatically increase or decrease with a span of 5 years)

WGIdevIneqPovDF2 <- kNN(WGIdevIneqPovDF, variable = "GNIperCapita", k = 5)
WGIdevIneqPovDF2 <- select(WGIdevIneqPovDF2, country, iso2c, date, stability, stabilityDummy, stabilityCategory, regulatoryQuality, region, subregion, GDPannualGrowthRate, GNIperCapita)


# the last step is to drop the observations with missing GDPgrowthRate (136)
...
# WGIdevIneqPovDF <- write.csv(WGIdevIneqPovDF2, "WGIdevIneqPovDF.csv")
# WGIdevIneqPovDF <- tbl_df(WGIdevIneqPovDF)
...
# creating a new variable for political change: either "democratization" if the polityScore increase from year1 to year1 + 1, "autocratization" if the polityScore decreas, or "no change" if it
...
# alternative coding working well and produced the new variable
polityScoreDf <- polityScoreDf %>% 
  group_by(country) %>% 
