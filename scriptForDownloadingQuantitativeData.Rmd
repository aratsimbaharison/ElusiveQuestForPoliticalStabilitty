...
# Note: the percentages of missing values are higher than 80% for the following variables. Therefore, it is reasonable to drop these variables, instead of imputing the missing values:
# - corruptionControl 
# - governmentEffectiveness
# - ruleOfLaw
# - voiceAndAccountability
# 
# However, by dropping these variables, we are left with only the following variables related to the WGI:
# - stability
# - stabilityDummy
# - stabilityCategory
# - regulatoryQuality (with 44 missing values, which should be imputed)

# the alternative way to get the WGI dataset (WGI2) is to download it directly from WGI Project homepage at: http://info.worldbank.org/governance/wgi/wgidataset_stata.zip. The stata format is readily formated in country-year dataframe and directly usable. 
...
# selecting the estimate values for the variables of interest
WGI2 <- select(WGI2, country = countryname, iso3c = code, date = year, stability = pve, regulatoryQuality = rqe, governmentEffectiveness = gee, voiceAndAccountability = vae, ruleOfLaw = rle, corruptionControl= cce)
...
## Joining the complete dataset WGI2 downloaded from the WGI Project home page with the UN classification into regions and subregions (script after 2/18/2019)
...
head(WGI2byCountryAndRegion)
# creating dummy variables for stability and category variables for stability
# creating a dummy variable for stability category with dplyr: "1" for stable country, "0" for unstable country
stabilityDf <- mutate(stabilityDf,
                      stabilityDummy = if_else(stabilityDf$stability > 0, "1", "0"))
head(stabilityDf)
...
WGI2byCountryAndRegion <- select(WGI2byCountryAndRegion, country, M49Code, iso2c, iso3c, date, stability, stabilityDummy, stabilityCategory, corruptionControl, governmentEffectiveness, regulatoryQuality, ruleOfLaw, voiceAndAccountability, region, subregion)
names(WGIbyCountryAndRegion)
summary(WGIbyCountryAndRegion)
...
## Downloading and Cleaning the Data on Development, Inequality, and Poverty from the World Bank Website Using `webstats`, and then joining the datasets with the dataset on WGI (WGIbyCountryAndRegion)
...
# creating a variable with four categories of development with dplyr
...
# dataset for HDI, downloaded directly from the UNDP Website (http://hdr.undp.org/en/data#), but it needs to be transformed and merged to World Bank data:
...
devIneqPovDF <- left_join(GNIperCapitaDf, GDPannualGrowthDF, by = c("country", "iso2c", "date")) %>%
  left_join(., UNDP_HDI2, by = c("country", "date")) %>%
  left_join(., GINIDf, by = c("country", "iso2c", "date")) %>%
  left_join(., povHeadCountDF, by = c("country", "iso2c", "date"))
...
# devIneqPovDF with imputation for GDPannualGrowthRate, HDI, GINI, and povertyHeadcount
devIneqPovDF2 <- kNN(devIneqPovDF)

devIneqPovDF2 <- select(devIneqPovDF2, country, iso2c, date, GNIperCapita, devCategory, GDPannualGrowthRate, HDI, GINI, povertyHeadCount)
head(devIneqPovDF2)
...
# Reading from the data set saved in the folder
...
# Using the dataset with imputation devIneqPovDF2:
WGIdevIneqPovDF2 <- left_join(devIneqPovDF2, WGIbyCountryAndRegion, by = c("country", "iso2c", "date"))
...
# Given the fact that there are 1009 missing values in most of the variables (particularly GNIperCapita, devCategory, GDPannualGrowthRate, HDI, GINI, povertyHeadCount) for in the dataset with imputation, I decided to rerun the imputation
WGIdevIneqPovDF2 <- kNN(WGIdevIneqPovDF2)
head(WGIdevIneqPovDF2)
names(WGIdevIneqPovDF2)
WGIdevIneqPovDF2 <- select(WGIdevIneqPovDF2, country, M49Code, iso2c, date, region, subregion, stability, stabilityDummy, stabilityCategory, regulatoryQuality, GNIperCapita, devCategory, GDPannualGrowthRate, HDI, GINI, povertyHeadCount)
summary(WGIdevIneqPovDF2)
...
# cleaning the dataset without imputation (WGIdevIneqPovDF). The process consists here of two steps:
# 1) dropping variables with more than 80% of missing values, that is GINI and povertyHeadCount, and
# 2) dropping the observations with missing values for the variables that have about 10% missing values

# dropping the columns with more than 80% of missing values: GINI 2674 out of 3650 obs, povertyHeadCount 2656 out of 3650 obs
WGIdevIneqPovDF <- select(WGIdevIneqPovDF, country, iso2c, date, stability, stabilityDummy, stabilityCategory, regulatoryQuality, region, subregion, GDPannualGrowthRate, GNIperCapita, devCategory, HDI)
summary(WGIdevIneqPovDF)

# the second step is to drop all remaining observations with missing values and using complete cases only
WGIdevIneqPovDF <- na.omit(WGIdevIneqPovDF)
summary(WGIdevIneqPovDF)

# # making the dataset on WGI, development, social inequality and poverty without imputation (WGIdevIneqPovDF), available in the local repository under the file name `WGIdevIneqPov.csv`
# 
# WGIdevIneqPov <- write.csv(WGIdevIneqPovDF, "WGIdevIneqPov.csv")
...
# # The data on revised combined polity score (UPP.REV.POL.XQ) in Polity IV Project can be downloaded from the World Bank databank website:
...
# However, the entire dataset on Polity IV Project can be directly downloaded from the Center for Systemic Peace website at: http://www.systemicpeace.org/inscrdata.html
...
# Cleaning the data on regime type

# step 1: remove 242 rows with NAs for Freedom House data
WGIregimeTypeByRegionDF <- WGIregimeTypeByRegionDF %>%
  filter(pr != "NA")

# Step 2: remove 482 rows with NAs for polity IV data
WGIregimeTypeByRegionDF <- WGIregimeTypeByRegionDF %>%
...
## Joining all dataframes on WGI, development, inequality, poverty, and regime types
...
# 2) WGIdevIneqPovDF2: with imputation for the GINI and povertyHeadcount
# 3) WGIregimeTypeByRegionDF

# Joining the dataframe without imputation

WGIdevRegimeTypeDF1 <- left_join(WGIdevIneqPovDF, WGIregimeTypeByRegionDF, by = c("country", "date"))

WGIdevRegimeTypeDF1 <-  select(WGIdevRegimeTypeDF1, country, M49Code, iso2c, region = region.x, subregion = subregion.x, date, stability = stability.x, stabilityDummy = stabilityDummy.x, stabilityCategory = stabilityCategory.x, regulatoryQuality = regulatoryQuality.x, GDPannualGrowthRate, GNIperCapita, devCategory, HDI, polityScore, polityCategory, politicalChange, democ, autoc, durable, xrreg, xrcomp, xropen, xconst, parreg, parcomp, exrec, exconst, polcomp, pr, cl, mean, status, inverse_pr, inverse_cl, inverse_mean, politicalChangeFH)

# Cleaning the dataframe WGIdevRegimeTypeDF1 by removing 207 rows with NAs for polityScore
WGIdevRegimeTypeDF1 <- WGIdevRegimeTypeDF1 %>%
  filter(polityScore != "NA")
summary(WGIdevRegimeTypeDF1)

# Joining the dataframe with imputation

WGIdevRegimeTypeDF2 <- left_join(WGIdevIneqPovDF2, WGIregimeTypeByRegionDF, by = c("country", "date"))

WGIdevRegimeTypeDF2 <- select(WGIdevRegimeTypeDF2, country, M49Code = M49Code.x, iso2c, region = region.x, subregion =  subregion.x, date, stability = stability.x, stabilityDummy = stabilityDummy.x, stabilityCategory = stabilityCategory.x, regulatoryQuality = regulatoryQuality.x, GNIperCapita, devCategory, GDPannualGrowthRate, HDI, GINI, povertyHeadCount, polityScore, polityCategory, politicalChange, democ, autoc, durable, xrreg, xrcomp, xropen, xconst, parreg, parcomp, exrec, exconst, polcomp, pr, cl, sum, mean, status, inverse_pr, inverse_cl, inverse_mean, politicalChangeFH)

# Cleaning the dataframe WGIdevRegimeTypeDF2 by removing 725 rows with NAs for polityScore
WGIdevRegimeTypeDF2 <- WGIdevRegimeTypeDF2 %>%
  filter(polityScore != "NA")
summary(WGIdevRegimeTypeDF2)


# # Making WGIdevRegimeTypeDF1 and WGIdevRegimeTypeDF2 available in the local repository under "WGIdevRegimeType1.csv" an "WGIdevRegimeType2.csv"
# 
# WGIdevRegimeType1 <- write.csv(WGIdevRegimeTypeDF1, "WGIdevRegimeType1.csv")
# WGIdevRegimeType2 <- write.csv(WGIdevRegimeTypeDF2, "WGIdevRegimeType2.csv")
# 
