...
- 01/25/2019: I decided to continue working on this project during my free times on the evenings and weekends.

- First thing to do is to review the data by reviewing the scripts and reloading from the data sources

- As I cleaned the data, I realize that too many variables that have too much missing values (up to 80%) had to be dropped. Consequently, I decide to imputation for variables that should not fluctuate too much over time (Ex.: GNI per capita, GINI, povertyHeadcount, ect.) and when it is reasonable to think that such imputation would not change the big picture.

- 01/27/2019: WGIbyCountryAndRegion (line 231 in the scriptForDownloadingQuantitativeData) is clean with no missing values, but the following variables had to be dropped: 
 corruptionControl
...
01/27/2019 at 6:30 am: I stopped at line 372 in the scriptForDownloadingQuantitativeData
